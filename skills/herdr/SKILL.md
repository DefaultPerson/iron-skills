...
name: herdr
description: >
  Control herdr — the terminal-native agent multiplexer — from inside a
  herdr-managed pane. Manage workspaces and tabs, split panes, spawn sibling
  agents (including more Claude Code instances), read another pane's output,
  run servers/tests in siblings, and block until output appears or another
  agent reaches a status — all via the `herdr` CLI talking to the running
  herdr instance over a local unix socket. Use ONLY when running inside herdr
  (environment variable HERDR_ENV=1); otherwise do not inspect or control the
  focused pane. Triggers: "herdr", "/herdr", "spawn an agent", "split a pane",
  "run in another pane", "wait for the server to be ready", "coordinate with
  another agent", "what is the other agent doing", "запусти в соседней панели",
  "спавни агента", "координируй агентов".
allowed-tools: Bash
---
...
- spawn more agent instances (including more Claude Code sessions)
...
every command below is a plain shell command — run each one through the **Bash** tool. a few conventions:

- structured commands (`workspace *`, `tab *`, `pane list/get/split`, `wait *`) print json on stdout; parse ids with `python3 -c` (shown in the recipes) or `jq` if present.
- `pane read` prints text, not json.
- `pane send-text`, `pane send-keys`, and `pane run` print nothing on success.
- to find your own pane without hardcoding an id, use `herdr pane current` (the pane this command runs in) or `herdr pane list` (the focused pane is yours).
- ids are compact and reused after closes — re-read them, never guess. see the ids note below.
...
the focused pane is yours. other panes are your neighbors. to get the id of the pane you are running in directly:
...
### spawn a new Claude Code agent and give it a task

interactive: launch `claude`, wait for its prompt, then hand it a task line.
...
headless one-shot: run `claude -p "<task>"` and wait for it to finish, then read the result. use this when you just want a delegated answer, not an interactive session.
...
herdr pane run "$NEW_PANE" 'claude -p "summarize open TODOs in src/ and print a checklist"'
herdr wait agent-status "$NEW_PANE" --status done --timeout 180000
herdr pane read "$NEW_PANE" --source recent --lines 120
```

you can spawn any other agent the same way (e.g. `codex`, `aider`) — just swap the command.
...
- `workspace list`, `workspace create`, `tab list`, `tab create`, `tab get`, `tab focus`, `tab rename`, `tab close`, `pane list`, `pane get`, `pane current`, `pane split`, `wait output`, and `wait agent-status` print json on success.
- `pane read` prints text, not json.
